@isset($model->$name)
    @if($model->$name)
        @foreach($model->$name as $k => $v)
            <label>{{ $label ?? '' }}</label>
            <div class="row dataset" style="margin-bottom: 10px">
                <div class="col">
                    <input type="text" class="form-control" name="{{ $name }}[{{ $k }}][title]" value="{{ $v->title }}" placeholder="Button Title">
                </div>
                <div class="col">
                    <input type="text" class="form-control" name="{{ $name }}[{{ $k }}][desc]" value="{{ $v->desc }}" placeholder="Button Short Desc">
                </div>
...
<label>{{ $label ?? '' }}</label>
<div class="row" data-name="{{ $name }}" style="margin-bottom: 10px">
    <div class="col">
...
<script src="{{ asset('admin/js/datasetButtons.js') }}"></script>
@endpushonce
